...
class Query_Processor:
	...
	def __init__(self, rank_method, default_index, no_phrases, query_list, **indices):
	...
	def process(self):
		...
		for q in qlist:
			full_normalized_query.update({q.id:{"single_term_index":[], "single_term_positional_index":[], "stem_index":[], "phrase_index":[]}})
	...
	def calculate_cosine_similarity(self, index, query):
		...
		for doc_idx in range(0, len(unnormalized_dot_product)):
			...
			final_cosine_similarity.update({unnormalized_dot_product[doc_idx][0]: top/bottom })
				...
				if next_query_term in idx.lexicon.keys():
					...
					for doc in q_term_doc_list:#for each doc for current term
						...
						if doc in doc_list_next_term:#check if next term exists in the same doc, if does not then a phrase is impossible
							position_list = position_list = self.get_position_list(index, q_term, doc)#get position list for current term
	...
	def prepare_results(self, query_num, ranking_dictionary):
		...
		for idx, each in enumerate(self.full_normalized_query.keys()):
			...
			if self.no_phrases == False:
				...
				if phrase_docs >= self.PHRASE_THRESHOLD:
					...
					search_results = rank_function("phrase_index", self.full_normalized_query[each]["phrase_index"])
					...
					if positional_docs >= self.POSITIONAL_TRESHOLD:
						...
						search_results = rank_function("single_term_positional_index", self.full_normalized_query[each]["single_term_positional_index"])
						...
						search_results = rank_function(self.default_index, self.full_normalized_query[each][self.default_index])
			...
			self.prepare_results(each, search_results)
...
def get_queries():
	...
	for line in queries:
		...
		if "<num>" in line:
			...
			number = current_number.split(" ")[2]
			query_text = " ".join(current_title.split(" ")[2:])

			new_query = Query(number, query_text)
...
def setup(command_args):
	...
	qp = Query_Processor(rank_method, default_index, no_phrases, get_queries(), single_term_index = load_index("single_term_index"), single_term_positional_index = load_index("single_term_positional_index"), stem_index = load_index("stem_index"), phrase_index = load_index("phrase_index"))
